...
router.get("/api/getDetails/:friendId", async (req: Request, res: Response) => {
  if (req.user) {
...
router.get("/friendReqs", async (req: Request, res: Response): Promise<void> => {
  if (req.user) {
    const userId = req.user.user_id;
    try {
...
router.post('/api/acceptRequest/:id', async (req, res) => {
  const { id } = req.params;
  if (!id) {
...
router.post('/api/rejectRequest/:id', async (req, res) => {
  const { id } = req.params;
  if (!id) {
    res.status(400).json({ message: 'Invalid request ID' });
    return;
...
router.delete("/api/removeFriend/:friendId", async (req: Request, res: Response) => {
